...
const Categorias = () => {
	const initialState = {
		avatar: 'https://pluspng.com/img-png/drinks-png-drink-png-6-png-image-1256.png',
		nombre: "",
		clase: ""
	}
	...
	const columns = [
		...
		{ field: 'clase', headerName: 'Clase', width: 220 },
		{
			field: '',
			headerName: 'Acciones',
			width: 200,
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/eliminar-categorias', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/guardar-categoria', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/editar-categorias', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
