...
const Usuarios = () => {
	const initialState = {
		avatar: 'https://i.pinimg.com/236x/f1/f5/15/f1f5153cabe32239c85842fb4d0ba3c8--ps.jpg',
		nombre: "",
		planeta: ""
	}
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/eliminar-usuario', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/guardar-usuario', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/editar-usuario', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
